...
const MightInterestYouComp: React.FC<Props> = ({ category }) => {
  ...
  const [problemArticle, setProblemArticle] = useState<Article[] | undefined>(
    undefined
  );
  ...
  const pathIncludesProblem = (
    param: string | undefined,
    problem: Article[] | undefined
  ) => {
  ...
  return (
    <div
      className={
        "bg-gradient-to-b from-[#E8FC97] to-[#9BFDF9] px-3 py-6 lg:py-10"
      }
    >
      {error && <div>{error}</div>}
      <h2 className={`text-center mx-auto`}>This might also interest you</h2>
      <div className={"flex gap-6 lg:gap-10 mt-4 lg:my-8 justify-center"}>
        {problemArticle &&
          problemArticle.map((a) => (
            <VerticalTeaserComp
