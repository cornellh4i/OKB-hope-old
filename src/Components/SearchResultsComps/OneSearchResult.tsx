...
const OneSearchResult: React.FC<Props> = ({article,url}) => {
  ...
  return (
    <Link className={'no-underline'} to={`${url}/${categorySlug}/${article.slug.current}`}>

    <div className={'grid grid-cols-2 gap-4 items-center text-black '}>
      <div>
        <img loading={"lazy"} className={'rounded'} src={urlFor(article.mainImage).height(300).width(400).url()}
             alt={article.title}/>
        {error && <div>{error}</div>}
      </div>
