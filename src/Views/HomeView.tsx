...
const HomeView = () => {
  ...
  const [homeViewContent, setHomeViewContent] =
    useState<null | HomeViewContent>(null);
  ...
  useEffect(() => {
    ...
    if (!feelings) {
      sanityClient
        .fetch(`*[_type == 'feelingsPreview' && page=='home']`)
    ...
    if (!inspirations) {
      sanityClient
        .fetch(`*[_type == 'getInspired' && page=='home']`)
        .then((data) => {
          const mappedData: Inspiration[] = data.map((d: Inspiration) => {
