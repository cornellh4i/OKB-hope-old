...
const SearchResultsView = () => {
  ...
  const [tipArticleTitles, setTipArticleTitles] = useState<null | Article[]>(
    null
  );
  ...
  return (
    <div className={"lg:max-w-screen-xl mx-auto p-3"}>
      ...
      <div>
        <span>{searchPhrase && `Results for ${searchPhrase}`}</span>
      </div>
      <div>{error && <div>{error}</div>}</div>
      <div className={"text-black"}>
        <div>
          {tipArticleTitles &&
            tipArticleTitles.map((a, idx) => (
              <div className={"my-3 w-full"} key={idx}>
