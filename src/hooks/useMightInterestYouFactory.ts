...
const useMightInterestYouFactory = (
  feeling: string | undefined,
  categoryObjects: CategoryObject[] | null
) => {
  const [mightInterestYou, setMightInterestYou] = useState<
    null | CategoryObject[]
  >(null);
  const [categoryObject, setCategoryObject] = useState<null | CategoryObject>(
    null
  );
